...
```{r}
exampleData <- dataGen(numTraj = 100, randomScale = .1, data.seed = 1)
...
```{r}
dtwMethodGrid <- simTool::expand_tibble(
  proc = "latrend",
  method = "lcMethodDtwclust",
  nClusters = 2:3,
  seed = 1,
  response = "Y"
)
...
```{r}
methodGrid <- dplyr::bind_rows(dtwMethodGrid, gckmMethodGrid)
head(methodGrid)
```
